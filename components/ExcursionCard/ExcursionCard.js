...
const ExcursionCard = ({
	...
	const [activeSchedule] = useState(0);
	const [activeDates] = useState(slide['variation'].map((item) => {
		return item['schedule'].map((time) => {
			return new Date(time['timeStart']).setHours(0, 0, 0, 0);
		});
	}).flat());
	...
	const prices = useCallback((item, isDesktop = false) => {
		return item['schedule'].map((time) => {
			return time['price'].map((price, index) => {
				return (
					<div className={styles.ExcursionCard__tariff} key={index}>
						...
						<div>
							...
							<strong>{price['amountSale']} ₽</strong>
	...
	useEffect(() => {
		slide['variation'].forEach((item) => {
			item['schedule'].forEach((time) => {
				time['price'].forEach((price) => {
					if (minPrice[0] && minPrice[1]) {
						if (minPrice[0] > price['amountSale']) setMinPrice([price['amountSale'], price['amount']]);
					} else {
						setMinPrice([price['amountSale'], price['amount']]);
					}
				});
			});
		});
	}, [slide, minPrice, setMinPrice]);
	
	return slide['variation'].map((item, index) => (
		<div
			className={`${styles.ExcursionCard} ${small ? styles.ExcursionCard_small : ''}`}
			key={index}
		>
			...
			<header className={styles.ExcursionCard__header}>
				...
				<span>
					...
					{item.duration > 60 ? `${Math.floor(item.duration / 60)}ч. ${item.duration % 60} мин.` : `${item.duration} мин.`}
				</span>
				<span>
					<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
						<g opacity="0.6">
				...
				<div className={styles.ExcursionCard__tariffs}>
					{activeSchedule === index && prices(item, true)}
				</div>
			</header>
			<div className={styles.ExcursionCard__departures}>
				...
				<div className={styles.ExcursionCard__times}>
					{item['schedule'].map((time, index) => {
						const isSameDate = activeDate.getDate() === new Date(time['timeStart']).getDate();
						const isSameMonth = activeDate.getMonth() === new Date(time['timeStart']).getMonth();
						const isSameYear = activeDate.getFullYear() === new Date(time['timeStart']).getFullYear();
						
						const isSameAllDate = isSameDate && isSameMonth && isSameYear;
						
						return isSameAllDate && (
							<button key={index}>{format(new Date(time['timeStart']), 'hh:mm')}</button>
						);
					})}
				...
				>
					<Calendar
						...
						tileDisabled={({date}) => activeDates.some((element) => element !== date.getTime())}
					/>
				</div>
			</div>
			<div className={styles.ExcursionCard__discount}>
				...
				<strong>От {minPrice[0]} ₽</strong>
				<span>{minPrice[1]} ₽</span>
			</div>
			<div className={styles.ExcursionCard__tariffs}>
				{activeSchedule === index && prices(item)}
			</div>
